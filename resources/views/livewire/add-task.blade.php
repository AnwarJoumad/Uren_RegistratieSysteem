                    ...
                    @foreach($tasks as $task)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                ...
                                <input placeholder="{{$task->name}}" type="text" wire:model="changeTask" style="border-radius: 0.375rem">
                                @else
                                {{$task->name}}
                                @endif
                            </th>
                            <td class="py-4 px-6">
                                ...
                                @else
                                    <button wire:click="changeTask({{ $task->id }})"  style="border: 2px solid blue" class="inline-block px-6 py-2 border-2 border-blue-600 text-blue-600 font-medium text-xs leading-tight uppercase rounded-full hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out">Taak aanpassen</button>
                                @endif
